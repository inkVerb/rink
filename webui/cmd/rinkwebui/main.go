...
func main() {
	cfgp := flag.String("config", "/opt/rink/conf/webui.conf", "config")
	flag.Parse()
	c := load(*cfgp)
	...
	log.Printf("rink web UI on %s", addr)
...
func (s *srv) logo(w http.ResponseWriter, r *http.Request) {
	for _, p := range []string{"web/static/pen-logo.svg", "/opt/rink/webui/web/static/pen-logo.svg"} {
		b, err := os.ReadFile(p)
		if err == nil {
...
func (s *srv) put(w http.ResponseWriter, ss *session) {
	...
	s.sess[tok] = ss
	s.mu.Unlock()
	http.SetCookie(w, &http.Cookie{Name: "rinkweb", Value: tok, Path: "/", HttpOnly: true, SameSite: http.SameSiteLaxMode, Secure: true})
}
...
func pamOK(user, pass string) bool {
	cmd := exec.Command("/opt/rink/webui/pam-auth")
...
func typeFromGroups(have []string) string {
	if hasAll(have, "rink", "admin") {
		return "admin"
	}
	if hasAll(have, "rink", "customer") {
		return "customer"
	}
	return ""
}
...
func (s *srv) login(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if !pamOK(u, pw) {
			...
			t := typeFromGroups(g)
			if t == "" {
				msg = `<p class="flash">Not a control-panel user (need groups rink + admin|customer).</p>`
			} else {
				db := s.c["db"]
	...
	fmt.Fprint(w, page("Control panel", "", msg+`<div class="card login"><h1>Sign in</h1>
<p class="muted">PAM user with groups <code>rink</code> and <code>admin</code> or <code>customer</code>. No Linux permissions.</p>
...
func (s *srv) issueEmail(db, user, email string) {
	...
	_ = os.WriteFile("/opt/rink/conf/webadmin/ot/"+user+".emailcode", []byte(code+"\n"), 0600)
}

func (s *srv) logout(w http.ResponseWriter, r *http.Request) {
	http.SetCookie(w, &http.Cookie{Name: "rinkweb", Value: "", Path: "/", MaxAge: -1})
...
func (s *srv) gate(w http.ResponseWriter, r *http.Request) {
		...
		case "changepw":
			...
			if len(np) >= 10 {
				...
				if cmd.Run() == nil {
					...
					_ = os.Remove("/opt/rink/conf/webadmin/ot/" + ss.User)
					ss.Stage = "email"
	...
	if !pwCh {
		...
		body += `<h2>Confirm email</h2><p class="muted">A code was written to <code>/opt/rink/conf/webadmin/ot/` + html.EscapeString(ss.User) + `.emailcode</code> and mailed if <code>mail</code> is configured.</p>
...
<form method="post"><input type="hidden" name="act" value="totp_confirm"><label>Code</label><input name="code" inputmode="numeric" required><p><button>Confirm Authenticator</button></p></form>`
	...
	body += `</div>`
	fmt.Fprint(w, page("Control panel", ss.User, body))
}
...
func (s *srv) home(w http.ResponseWriter, r *http.Request) {
	...
	info, _ := exec.Command("/opt/rink/run/listvps").CombinedOutput()
	fmt.Fprint(w, page("Control panel", ss.User+" · "+ss.Type, `<div class="card"><h1>Server</h1><pre>`+html.EscapeString(string(info))+`</pre></div>
...
func (s *srv) serf(w http.ResponseWriter, r *http.Request) {
	...
	case "setipv4", "setipv6", "setipv4update", "setipv6update":
		fmt.Fprint(w, page("Control panel", ss.User, `<div class="card"><p class="flash">IP configuration is rink-only.</p></div>`))
		return
	}
	args := strings.Fields(r.FormValue("args"))
	cmd := exec.Command(append([]string{"/opt/rink/run/" + name}, args...)...)
	out, err := cmd.CombinedOutput()
	msg := html.EscapeString(string(out))
	if err != nil {
		msg += "\n" + html.EscapeString(err.Error())
	}
	fmt.Fprint(w, page("Control panel", ss.User, `<div class="card"><h2>`+html.EscapeString(name)+`</h2><pre>`+msg+`</pre><p><a href="/">Back</a></p></div>`))
}
...
func (s *srv) oauth(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprint(w, page("Control panel", ss.User, `<div class="card"><h2>Link `+html.EscapeString(prov)+`</h2>
<p class="muted">Set client IDs in <code>webui.conf</code> for Apple, Google, Facebook, LinkedIn, GitHub, and X. Linking stores the subject against this PAM user; it never becomes a Linux account.</p></div>`))
}
